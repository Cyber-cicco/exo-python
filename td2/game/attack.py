...
class Affliction:
    ...
    def __init__(self, name:str, effect:AfflictionFunc, chances_working:int, min_turns:int, max_turns:int, recovery_chance:int) -> None:
        self.name = name
    ...
    def inflict(self, pokemon:"Pokemon", combat_turn:int, diags:list[str]) -> list[str]:
...
class PokemonAffliction(Affliction):
    ...
    def __init__(self, aff:Affliction, combat_turn:int):
        super().__init__(aff.name, aff.__effect, aff.chances_working, aff.min_turns, aff.max_turns, aff.recovery_chance)
        self.combat_turn = combat_turn


class Attack:

    def __init__(self, nom:str, atk_type:AttackType, base_pwr:int, pwr_mult:int, crit_chance:int=10, target:AttackTarget=AttackTarget.ENNEMY, afflictions:list[Affliction]=[]):
        self.nom = nom
